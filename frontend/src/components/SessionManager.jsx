...
import { listSessions, createSession, clearSessionDocuments } from '../api/client'
      ...
      {!isCreating ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            ...
            <button
              onClick={async () => {
                const confirmed = window.confirm("Are you sure you want to clear all documents in this session?");
                if (!confirmed) return;
                try {
                  setLoading(true)
                  await clearSessionDocuments(currentSessionId)
                  await fetchSessions()
                  // Refresh active data
